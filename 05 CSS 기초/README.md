...
## CSS3와 CSS 모듈
- 이전 CSS2나 CSS1보다 정교하고 화려한 화면을 구성할 수 있고 애니메이션까지 지원합니ㅏㄷ. 
- CSS2 규약 안에는 스타일과 관련된 것들이 한꺼번에 담겨 있기 때문에 덩치가 크고 복잡해 한 번에 업데이트하기 쉽지 않다는 단점이 있었습니다.
- CSS3 부터는 배경이나 글꼴, 박스 모델 등 수십 개 기능을 주제별로 규약울 따로 만들었습니다. 이것을 <code>CSS 모듈</code>이라고 합니다.
- 모듈 중 일부는 완성된 것도 있지만 아직 개발 중인 것도 있습니다. 그에 따라 브라우저에 따라 지원되는 기능도 있고 지원되지 않는 기능도 있습니다. 
- 이처럼 모듈별로 진행속도도 다르고 필요에 따라 새로운 모듈이 생기기 때문에 CSS3는 한번에 표준 규약이 결정되지 않습니다. 다시 말해 CSS3는 CSS3 표준이라고 하나로 묶어 말할 수 없고 계속 CSS 모듈이 등장할 수 있습니다.

## CSS3와 브라우저 접두사(prefix)

- 아직 표준 규약이 아닌 속성들은 브라우저에 따라 다른 방식으로 지원되기 때문에 속성 이름 앞에 접두사(prefix)를 붙여 브라우저별로 구분해야 합니다.
- 표준 규약이 만들어졌더라도 이전 버번의 모던 브라우저 사용자를 고려해 계속 브라우저 접두사를 붙여 사용하기도 합니다.
...
- 브라우저 접두사를 붙여 사용할 떄는 각 브라우저 접두사를 붙인 속성을 먼저 쓰고 표준이 정해진 후 사용할 속성을 마지막에 사용합니다.
- 나중에 표준 속성이 결정된 수 앞에 썼던 브라우저 접두사가 붙은 속성들만 지우면 됩니다. 

```css
-wekbit-colum-count: 3; /* 사파리와 크롬 브라우저용 속성 */
